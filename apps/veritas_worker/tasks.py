...
def run_job(job_id: str):
    ...
    def upd(state: str, stage: str, prog: float, **kw):
        (RUNS / job_id / "status.json").write_text(
            json.dumps(
                {"state": state, "stage": stage, "progress": prog}
                | (kw.get("extra") or {})
            ),
            encoding="utf-8",
        )
    ...
    for vid in vids:
        ...
        if err:
            missing.append({"video_id": vid, "error": err})
            continue
        for seg in tr:
            ...
            items.append(
                {
                    "id": f"{vid}-{int(seg['start'] * 1000)}",
                    "url": url,
                    "text": t,
                    "meta": {"video_id": vid, "timestamp_ms": int(seg["start"] * 1000)},
                }
            )
    ...
    results = {
        ...
        "fracture_score": 0.0,
        "unity_bridges": [],
        "run_folder": str(RUNS / job_id),
    }
    if missing:
        results["notes"] = {"missing_transcripts": missing}
