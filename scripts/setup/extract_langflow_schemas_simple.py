    ...
    if inputs_section:
        ...
        for match in input_matches:
            ...
            display_name_match = re.search(
                r'display_name\s*=\s*["\']([^"\']+)["\']', input_params
            )
...
def extract_component_schemas(langflow_path: str) -> Dict[str, Any]:
    ...
    schemas = {}
    components_dir = (
        Path(langflow_path) / "src" / "backend" / "base" / "langflow" / "components"
    )
    ...
    for component_name, relative_path in target_components.items():
        ...
        try:
            ...
            if component_info:
                ...
                print(
                    f"  ✅ Extracted {component_name}: {component_info.get('display_name', component_name)}"
                )
            else:
