    ...
    def detect_spike_anomaly(
        ...
        if deviation > self.threshold_std:
            ...
            return AnomalyDetection(
                ...
                severity=severity,
                anomaly_type="spike",
                predicted_impact=f"Metric {metric_type} for {service_name} shows {deviation_percentage:.1f}% deviation from baseline",  # noqa: E501
    ...
    def detect_trend_anomaly(
        ...
        if abs(slope) > 0.1 and r_squared > 0.7:  # Adjustable thresholds
            ...
            return AnomalyDetection(
                ...
                severity=severity,
                anomaly_type="trend",
                predicted_impact=f"Strong {trend_direction} trend detected in {metric_type} for {service_name}",  # noqa: E501
                timestamp=datetime.now(),
            )
...
class PredictiveModel:
    ...
    def predict_value(
        self, service_name: str, metric_type: str, horizon_minutes: int = 5
    ) -> Optional[Prediction]:
...
class PredictiveMonitor:
    ...
    def _collect_and_analyze_metrics(self):
        ...
        for service_name, service_metrics in metrics.items():
            for metric_type, metric_value in service_metrics.items():
                ...
                self.predictive_model.update_model(
    ...
    def _handle_anomaly(self, anomaly: AnomalyDetection):
        ...
        self.logger.warning(
            f"Anomaly detected: {anomaly.service_name}/{anomaly.metric_type} - {anomaly.severity}"  # noqa: E501
        )
    ...
    def _handle_prediction(self, prediction: Prediction):
        ...
        self.logger.info(
            f"Prediction generated: {prediction.service_name}/{prediction.metric_type}"
        )
        ...
        if self._check_slo_breach_prediction(prediction):
            alert = Alert(
                alert_type="webhook",
                endpoint="",
                message=f"SLO breach predicted: {prediction.service_name}/{prediction.metric_type}",  # noqa: E501
                severity="high",
                timestamp=datetime.now(),
            )
            self.alert_manager.send_alert(alert)
    ...
    def get_monitoring_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            critical_anomalies = [
                a for a in recent_anomalies if a["severity"] == "critical"
            ]
            ...
            return {"error": str(e), "monitoring_active": self.monitoring_active}
