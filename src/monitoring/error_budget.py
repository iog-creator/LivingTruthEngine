...
class ErrorBudgetMonitor:
    ...
    def record_metric(self, service_name: str, metric_type: str, metric_value: float) -> int:
        """
        ...
        try:
            with self.get_db_connection() as conn:
                with conn.cursor() as cursor:
                    cursor.callproc('lte.record_error_budget_metric', 
                                   [service_name, metric_type, metric_value])
                    result = cursor.fetchone()
                    metric_id = result[0] if result else None
                    
                    self.logger.info(f"Recorded metric: {service_name}/{metric_type}={metric_value}")
                    return metric_id
    ...
    def get_error_budget_status(self, service_name: str, metric_type: str, 
        ...
        try:
            with self.get_db_connection() as conn:
                with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                    cursor.callproc('lte.calculate_error_budget_consumption', 
                    ...
                    if result:
                        return {
                            ...
                            'error_budget_consumed': float(result['error_budget_consumed']),
                            'error_budget_remaining': float(result['error_budget_remaining']),
    ...
    def record_recovery_action(self, action_type: str, service_name: str, 
        ...
        try:
            with self.get_db_connection() as conn:
                with conn.cursor() as cursor:
                    cursor.callproc('lte.record_recovery_action', [
                    ...
                    self.logger.info(f"Recorded recovery action: {action_type} for {service_name}")
                    return action_id
    ...
    def record_chaos_test_result(self, test_type: str, test_duration_ms: int,
        ...
        try:
            with self.get_db_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    cursor.callproc('lte.record_chaos_test_result', [
                        test_type, test_duration_ms, recovery_time_ms,
                        recovery_success, slo_violation, details_json
                    ])
                    result = cursor.fetchone()
    ...
    def check_error_budget_threshold(self, service_name: str, metric_type: str,
                                    threshold_percentage: float = 80.0) -> bool:
        """
        ...
        try:
            ...
            if 'error' in budget_status:
                self.logger.warning(f"Error getting budget status: {budget_status['error']}")
                return False
