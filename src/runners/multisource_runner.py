...
class MultiSourceRunner:
    ...
    async def check_health_gates(self, sources: List[str]) -> Dict[str, bool]:
        ...
        for source_type in sources:
            ...
            try:
                ...
                logger.info(f"Health gate for {source_type} ({tool_name}): {health_results[source_type]}")
        ...
        try:
            ...
            for i, result in enumerate(results):
                ...
                if isinstance(result, Exception):
                    ...
                    logger.info(f"Source {source_type} completed with {len(result)} documents")
            ...
            if all_documents:
                try:
                    ...
                    logger.info(f"Stored {len(all_documents)} documents in pgvector for job {job.job_id}")
                except Exception as e:
            ...
            logger.info(f"Job {job.job_id} completed with {aggregated_results['total_documents']} total documents")
    ...
    async def _process_source(self, source_config: SourceConfig) -> List[Dict[str, Any]]:
        ...
        device = gpu_scheduler.get_optimal_device("embedding")  # Most sources need embeddings
        gpu_scheduler.log_device_allocation("embedding", device, f"for {source_type} processing")
    ...
    async def _mock_youtube_adapter(self, params: Dict[str, Any]) -> List[Dict[str, Any]]:
            ...
            async with httpx.AsyncClient(timeout=5) as client:
                ...
                if gate_name == "pgvector":
                    ...
                    if response.status_code == 200:
                        data = response.json()
                        return data.get("data", {}).get("pgvector", {}).get("enabled", False)
                elif gate_name == "rulego":
                    # Check rulego health
                    response = await client.get("http://localhost:8050/api/health/full")
                    if response.status_code == 200:
                        data = response.json()
                        return data.get("data", {}).get("rulego", {}).get("status") == "ok"
                elif gate_name == "lmstudio":
                    # Check LM Studio health
                    response = await client.get("http://localhost:8050/api/health/full")
                    if response.status_code == 200:
                        data = response.json()
                        return data.get("data", {}).get("gates", {}).get("lm_studio", False)
                else:
