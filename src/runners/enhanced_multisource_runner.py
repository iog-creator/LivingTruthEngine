...
class EnhancedMultiSourceRunner:
    ...
    async def start_job(
        self, sources: List[Dict[str, Any]], job_label: Optional[str] = None
    ) -> str:
        ...
        logger.info(
            f"Started enhanced multi-source job {job_id} with {len(sources)} sources"
        )
        return job_id
    ...
    async def _process_job(self, job_id: str):
        ...
        try:
            # Check health gates
            health_status = await self.check_health_gates(
                [s["type"] for s in job.sources]
            )
            if not all(health_status.values()):
                failed_gates = [
                    gate for gate, status in health_status.items() if not status
                ]
            ...
            job.deduplication_stats["duplicates"] = job.deduplication_stats[
                "total"
            ...
            logger.info(
                f"Job {job_id} completed: {len(unique_documents)} unique documents from {len(all_documents)} total"  # noqa: E501
            )
    ...
    def _deduplicate_across_sources(self, documents: List[Any]) -> List[Any]:
        ...
        for doc in documents:
            if doc.sha256 not in seen_hashes:
                ...
                logger.info(
                    f"Cross-source duplicate detected: {doc.title} (hash: {doc.sha256[:8]}...)"  # noqa: E501
                )

        return unique_docs

    async def _persist_job_results(
        self, job: EnhancedMultiSourceJob, documents: List[Any]
    ):
        ...
        try:
            ...
            logger.info(
                f"Persisting job results for {job.job_id}: {len(documents)} documents"
            )
