...
class GPUScheduler:
        ...
        try:
            ...
            if not available_gpus:
                ...
                self._log_fallback_event(
                    "gpu_vram_low", "Insufficient VRAM on all GPUs", gpus[0]
                )
                return {
                    "available": False,
                    "reason": f"Insufficient VRAM (threshold: {self.vram_threshold_mb}MB)",  # noqa: E501
    ...
    def reserve_gpu_memory(self, required_mb: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if available_mb < required_mb:
                ...
                return {
                    "success": False,
                    "reason": f"Insufficient VRAM: required {required_mb}MB, available {available_mb}MB",  # noqa: E501
                    "fallback_required": True,
                }
    ...
    def simulate_low_vram(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self._log_fallback_event(
                "gpu_vram_low", "Simulated low VRAM condition", None
            )
        ...
        return [
            {
                ...
                "type": event.type,
                "reason": event.reason,
                "gpu_info": self._gpu_to_dict(event.gpu_info)
                if event.gpu_info
                else None,
    ...
    def _log_fallback_event(
        self, event_type: str, reason: str, gpu_info: Optional[GPUInfo]
    ):
