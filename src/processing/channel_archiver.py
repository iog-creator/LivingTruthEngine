...
class ChannelArchiver:
    ...
    def _emit_telemetry(self, event: str, payload: Dict[str, Any]) -> None:
        try:
            ...
            self.telemetry_status_file.write_text(json.dumps(status_snapshot, indent=2, ensure_ascii=False), encoding="utf-8")
        except Exception as e:
        ...
        try:
            ...
            if self.notebook_agent:
                try:
                    ...
                    transcript_tool = create_youtube_transcript_tool(str(self.sources_dir))
                    ...
                    if "❌" in result or "No subtitles found" in result:
                        logger.warning(f"Failed to get transcript for {title}: {result}")
                    ...
                    if transcript_file.exists():
                        ...
                        try:
                            ...
                            logger.warning(f"Organization step failed for {title}: {organize_error}")
            ...
            while i < len(lines):
                ...
                if '-->' in line:
                    ...
                    while i < len(lines) and lines[i].strip() and '-->' not in lines[i]:
                        ...
                        if lines[i].strip().isdigit() or lines[i].strip().upper() == 'WEBVTT':
                            i += 1
                            continue
    ...
    def _save_organized_assets(
        ...
        organized_transcript.write_text(transcript_file.read_text(encoding='utf-8'), encoding='utf-8')
        ...
        if vtt_file and vtt_file.exists():
            ...
            if self.save_mode == "full":
                ...
                try:
                    organized_vtt_path.write_text(vtt_file.read_text(encoding='utf-8'), encoding='utf-8')
                except Exception as e:
        ...
        record: Dict[str, Any] = {
            ...
            "paths": {
                ...
                "segments": str(video_dir / "transcript_segments.json") if segments else None,
        ...
        if not annotations_path.exists():
            annotations_path.write_text(
                json.dumps({"tags": [], "entities": [], "links": []}, indent=2, ensure_ascii=False),
                encoding='utf-8'
            )
        ...
        try:
            ...
            index_data = [item for item in index_data if item.get("video_id") != video_info.id]
            index_entry = {
                "video_id": video_info.id,
                "title": video_info.title,
            ...
            organized_index.write_text(json.dumps(index_data, indent=2, ensure_ascii=False), encoding='utf-8')
        except Exception as e:
    ...
    def organize_existing_archives(self) -> Dict[str, Any]:
        ...
        for v in videos:
            ...
            video_info = VideoInfo(id=video_id, title=title, url=url, duration=duration, upload_date=upload_date, view_count=view_count, description="")
            transcript_file = self.sources_dir / f"{video_id}_transcript.txt"
            ...
            vtt_file = self._find_vtt_file(video_id)
            try:
                self._save_organized_assets(video_info, transcript_file, vtt_file, {"status": "success", "archived_at": v.get("archived_at")})
                summary["processed"] += 1
            except Exception as e:
                logger.warning(f"Organize existing failed for {video_id}: {e}")
        return summary

    def rebuild_organized_index(self, channel_url: Optional[str] = None) -> Dict[str, Any]:
    ...
    def archive_channel(self, channel_url: str, max_videos: Optional[int] = None) -> ChannelArchiveSummary:
        ...
        try:
            ...
            self._set_channel_scope(channel_url)
            # Start telemetry
            self._emit_telemetry("archive_start", {"channel_url": channel_url, "message": "Starting archive"})
            ...
            logger.info(f"Successfully archived: {successful_archives}/{len(videos)} videos")
    ...
    def build_channel_knowledge_base(self) -> str:
        ...
        try:
            ...
            successful_videos = [v for v in archive_data['videos'] if v['status'] == 'success']
            ...
            knowledge_base += f"**Successfully Archived**: {archive_data['successful_archives']}\n\n"
            ...
            for video in successful_videos:
                ...
                knowledge_base += f"- **Duration**: {video.get('duration', 'Unknown')} seconds\n"
                knowledge_base += f"- **Upload Date**: {video.get('upload_date', 'Unknown')}\n"
            ...
            for video in videos:
                try:
                    ...
                    if result.status == "success" and result.transcript_length:
                        transcript_file = self.sources_dir / f"{video.id}_transcript.txt"
                        if transcript_file.exists():
                        ...
                        logger.warning(f"Failed to download transcript for: {video.title} - {result.error}")
