...
"""
    ...
    # ============================================================================
    # mcp.lte.diagnostics namespace - MCP Server Diagnostics
    # ============================================================================
    ...
    def mcp_router_diagnostics(
        ...
        try:
            ...
            try:
                ...
                return envelope_err("server_start_failed", str(e))

            rpc = JsonRpcIO(proc, timeout=timeout)
            try:
                ...
                return envelope_err("diagnostics_failed", repr(e))
            finally:
                try:
                    proc.terminate()
    ...
    @mcp.tool()
    def get_ssot_meta_index(self) -> Dict[str, Any]:
        """Return the full SSOT metadata index (generated by meta-index)."""
        idx = Path("reports/ssot_meta_index.json")
        if not idx.exists():
            raise FileNotFoundError("reports/ssot_meta_index.json not found. Run `make meta-index`.")
        try:
            return {"status": "ok", "data": json.loads(idx.read_text(encoding="utf-8"))}
        except json.JSONDecodeError as e:
            raise ValueError(f"Invalid JSON in meta index: {e}")  # specific, fail-fast

    @mcp.tool()
    def search_ssot_meta(self,
                         owner: Optional[str] = None,
                         tag: Optional[str] = None,
                         severity: Optional[str] = None,
                         text: Optional[str] = None) -> Dict[str, Any]:
        """Filter the SSOT meta index by owner/tag/severity/text."""
        idx = Path("reports/ssot_meta_index.json")
        if not idx.exists():
            raise FileNotFoundError("reports/ssot_meta_index.json not found. Run `make meta-index`.")
        meta = json.loads(idx.read_text(encoding="utf-8"))
        rules: List[Dict[str, Any]] = meta.get("rules", [])
        docs: List[Dict[str, Any]] = meta.get("docs", [])
        def match(item: Dict[str, Any]) -> bool:
            smetas = item.get("ssot_meta", []) or []
            hay = (json.dumps(item, ensure_ascii=False) if text else "")
            # Check each embedded YAML block; succeed if any one matches
            def blk_ok(b: Dict[str, Any]) -> bool:
    ...
    @mcp.tool()
    def get_rule_integrity_status(self) -> Dict[str, Any]:
        """Quick integrity glance: how many rules, with/without ssot_meta, and last generated time."""
        idx = Path("reports/ssot_meta_index.json")
        if not idx.exists():
            raise FileNotFoundError("reports/ssot_meta_index.json not found. Run `make meta-index`.")
        meta = json.loads(idx.read_text(encoding="utf-8"))
        rules = meta.get("rules", [])
        with_meta = sum(1 for r in rules if r.get("ssot_meta"))
        without_meta = len(rules) - with_meta
        return {
            "status": "ok",
            "data": {
                "generated_at_utc": meta.get("generated_at_utc"),
                "counts": {"rules_total": len(rules), "rules_with_meta": with_meta, "rules_without_meta": without_meta},
            },
        }
