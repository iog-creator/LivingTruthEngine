...
class LangflowMCP:
    ...
    def query_langflow(
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return f"✅ Langflow query successful:\n\n{result.get('text', 'No response text')}"  # noqa: E501
            else:
    ...
    def get_langflow_status(self) -> str:
        ...
        try:
            ...
            project_url = (
                f"{self.langflow_api_endpoint}/api/v1/mcp/project/{self.project_id}/sse"
            )
            ...
            status_info = {
                ...
                "main_endpoint_status": "✅ Connected"
                if main_response.status_code == 200
                else f"❌ Failed ({main_response.status_code})",
                "project_endpoint_status": "✅ Connected"
                if project_response.status_code == 200
                else f"❌ Failed ({project_response.status_code})",
    ...
    def list_langflow_tools(self) -> str:
        ...
        try:
            ...
            url = f"{self.langflow_api_endpoint}/api/v1/mcp/project/{self.project_id}/tools"  # noqa: E501
    ...
    def create_langflow(
        self, flow_config: Dict[str, Any], flow_id: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        if os.getenv("LANGFLOW_WRITE_ENABLED", "false").lower() != "true":
            logger.warning(
                "Langflow write operations disabled (LANGFLOW_WRITE_ENABLED=false)"
            )
            return {
                "status": "skipped",
                "reason": "Write operations disabled",
        ...
        required_fields = [
            "name",
            "data",
        ]  # Verified: 'name' and 'data' (nodes/edges) required
        ...
        try:
            ...
            logger.debug(
                "Sending %s request to %s with config: %s", method, url, flow_config
            )
            response = requests.request(
                method, url, json=flow_config, headers=headers, timeout=10
            )
            ...
            logger.info(
                "Workflow %s: %s",
                "updated" if flow_id else "created",
                result.get("id", "unknown"),
            )
            return result  # Returns full flow object, e.g., {"id": "...", "name": "...", "data": {...}}  # noqa: E501
            ...
            logger.error(
                "HTTP error from Langflow API: %s - Response: %s",
                e,
    ...
    def export_flow_to_file(
        self, flow_id: str, file_path: str = "data/flows/exported_flow.json"
    ) -> str:
        ...
        try:
            ...
            response = requests.get(
                f"{self.langflow_api_endpoint}/api/v1/flows/{flow_id}",
                headers=headers,
                timeout=10,
            )
            response.raise_for_status()
    ...
    def configure_node_in_flow(
        self, flow_json: Dict[str, Any], node_id: str, config_params: Dict[str, Any]
    ) -> Dict[str, Any]:
                    ...
                    for key, value in config_params.items():
                        if key in node["data"]["node"]["template"]:
                            ...
                            if param.get("required", False) and not value:
                                raise ValueError(
                                    f"Required param {key} cannot be empty"
                                )
                            param["value"] = value
                    logger.info(
                        f"Configured node {node_id} with params: {config_params}"
                    )
                    break
            else:
    ...
    def import_flow_from_json(
        self, flow_json: Dict[str, Any], flow_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """Import JSON to Langflow via API (create/update)."""
        return self.create_langflow(flow_json, flow_id)

    def save_flow_to_file(
        self, flow_json: Dict[str, Any], file_path: str = "data/flows/updated_flow.json"
    ) -> str:
...
@mcp.tool()
def create_langflow(
    flow_config: Dict[str, Any], flow_id: Optional[str] = None
) -> Dict[str, Any]:
...
@mcp.tool()
def export_flow_to_file(
    flow_id: str, file_path: str = "data/flows/exported_flow.json"
) -> str:
    """Export flow to JSON file for editing."""
...
@mcp.tool()
def configure_node_in_flow(
    flow_json: Dict[str, Any], node_id: str, config_params: Dict[str, Any]
) -> Dict[str, Any]:
    """Configure node in loaded flow JSON."""
...
@mcp.tool()
def import_flow_from_json(
    flow_json: Dict[str, Any], flow_id: Optional[str] = None
) -> Dict[str, Any]:
    """Import JSON to Langflow via API (create/update)."""
...
@mcp.tool()
def save_flow_to_file(
    flow_json: Dict[str, Any], file_path: str = "data/flows/updated_flow.json"
) -> str:
    """Save modified flow to file for track/verification."""
