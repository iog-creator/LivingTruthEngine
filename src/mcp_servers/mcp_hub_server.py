...
class MCPHubServer:
    ...
    def __init__(self):
        ...
        self.loaded_modules = {}
        logger.info(
            f"MCP Hub Server initialized with {self.registry.get('total_tools', 0)} tools"
        )
    ...
    def validate_registry(self, registry: Dict[str, Any]) -> bool:
            ...
            for tool in server_data["tools"]:
                ...
                for field in required_tool_fields:
                    if field not in tool:
                        validation_errors.append(
                            f"Tool '{tool.get('name', 'unknown')}' missing required field: {field}"
                        )
                    ...
                    if not isinstance(tool["params_schema"], dict):
                        ...
                        for param_name, param_def in tool["params_schema"].items():
                            if not isinstance(param_def, dict):
                                validation_errors.append(
                                    f"Tool '{tool['name']}' parameter '{param_name}' definition must be a dictionary"
                                )
                                ...
                                validation_errors.append(
                                    f"Tool '{tool['name']}' parameter '{param_name}' has invalid type '{param_def['type']}'. Valid types: {VALID_TYPES}"
                                )
        ...
        logger.info(
            f"Registry validation passed for {sum(len(server.get('tools', [])) for server in registry['servers'].values())} tools"
        )
        return True
    ...
    def batch_execute_tools(self, tools: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        logger.info(
            f"Batch executed {len(tools)} tools in {total_duration:.2f}s, {successful_count} successful"
        )
    ...
    def build_tool(self, tool_def: Dict[str, Any]) -> str:
        ...
        for param_name, param_def in tool_def["params_schema"].items():
            if "type" not in param_def:
                ...
                raise ValueError(
                    f"Parameter '{param_name}' has invalid type '{param_def['type']}'. Valid types: {VALID_TYPES}"
                )
