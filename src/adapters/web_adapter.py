...
class WebAdapter(BaseAdapter):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if LEGACY_WEB_AVAILABLE:
            ...
            logger.warning(
                "Legacy web fetcher not available, using mock implementation"
            )
    ...
    async def _fetch_with_legacy_fetcher(
        ...
        try:
            ...
            for url in urls:
                try:
                    ...
                    internal_docs = await self._extract_web_internal(
                        url, max_depth, js_render
                    )
                    if internal_docs:
                        documents.extend(internal_docs)
                        logger.info(
                            f"Internal web extraction successful for {url}: {len(internal_docs)} documents"  # noqa: E501
                        )
                        continue

                    # Fallback to enhanced extraction (secondary fallback)
                    logger.warning(
                        f"Internal extraction failed for {url}, trying enhanced extraction"  # noqa: E501
                    )
                    ...
                    logger.warning(
                        f"Enhanced extraction failed for {url}, falling back to legacy fetcher"  # noqa: E501
                    )
                    raw_docs = self.legacy_fetcher.fetch_url(
                        url, crawl_depth=crawl_depth, max_depth=max_depth
                    )
                    ...
                    for raw_doc in raw_docs:
                        try:
                            ...
                            doc = {
                                "id": raw_doc.get(
                                    "id", f"web_{hash(raw_doc.get('url', '')) % 10000}"
                                ),
                                "source": "web",
                                ...
                                "metadata": {
                                    ...
                                    "content_type": raw_doc.get("meta", {}).get(
                                        "content_type", ""
                                    ),
                                    "crawl_depth": crawl_depth,
                                    "js_render": js_render,
                                    "extraction_method": raw_doc.get("meta", {}).get(
    ...
    async def _extract_web_internal(
        self, url: str, max_depth: int, js_render: bool
    ) -> List[Dict[str, Any]]:
            ...
            for raw_doc in raw_docs:
                try:
                    ...
                    doc = {
                        "id": raw_doc.get(
                            "id",
                            f"web_internal_{hash(raw_doc.get('uri', url)) % 10000}",
                        ),
                        "source": "web",
                        ...
                        "metadata": {
                            ...
                            "links_count": raw_doc.get("meta", {}).get(
                                "links_count", 0
                            ),
                            "extraction_method_detail": raw_doc.get("meta", {}).get(
    ...
    async def _extract_web_enhanced(
        self, url: str, js_render: bool
        ...
        try:
            ...
            # Fetch content
            headers = {"User-Agent": "LivingTruthEngine/1.0 (Phase 9.5.0a Enhanced)"}
            ...
            try:
                ...
                extracted_text = trafilatura.extract(
                    html_content, include_formatting=True
                )
        ...
        try:
            ...
            async with async_playwright() as p:
                ...
                try:
                    ...
                    extracted_text = trafilatura.extract(
                        content, include_formatting=True
                    )
    ...
    async def _fetch_mock_documents(
        self, params: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        for i, url in enumerate(urls[:3]):  # Max 3 mock documents
            try:
                ...
                doc = {
                    ...
                    "text": f"This is mock content from {domain}. It contains sample text that would normally be extracted from a web page using {'JavaScript rendering' if js_render else 'static HTML parsing'} with crawl depth {max_depth}.",  # noqa: E501
                    "metadata": {
                        "domain": domain,
                        "content_type": "text/html",
                        "crawl_depth": 0,
