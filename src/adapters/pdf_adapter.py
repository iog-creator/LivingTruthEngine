...
try:
    from ..ingestion_general.extractors.pdf_extractor import (
        PDFExtractor as LegacyPDFExtractor,
    )
...
class PDFAdapter(BaseAdapter):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if LEGACY_PDF_AVAILABLE:
            ...
            logger.warning(
                "Legacy PDF extractor not available, using mock implementation"
            )
    ...
    async def _fetch_with_legacy_extractor(
        ...
        try:
            ...
            for url in urls:
                try:
                    ...
                    logger.warning(
                        f"Enhanced extraction failed for {url}, falling back to legacy extractor"  # noqa: E501
                    )
                    try:
    ...
    async def _extract_pdf_internal(
        self, url: str, ocr_required: bool
        ...
        try:
            ...
            result = self.legacy_extractor.extract_text(
                pdf_content=pdf_content, ocr_mode=ocr_mode, filename=filename
            )
            ...
            if result and result.get("text") and len(result["text"].strip()) >= 100:
                return {
                    ...
                    "metadata": {
                        ...
                        "extraction_method_detail": result.get(
        ...
        for i, url in enumerate(urls[:3]):  # Max 3 mock documents
            try:
                ...
                doc = {
                    ...
                    "text": f"This is mock content extracted from {filename}. It contains sample text that would normally be extracted from a PDF document using {'OCR processing' if ocr_required else 'direct text extraction'}.",  # noqa: E501
                    "metadata": {
