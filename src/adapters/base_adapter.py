    ...
    def _filter_duplicates(self, documents: List[DocumentLike]) -> List[DocumentLike]:
        ...
        for doc in documents:
            if doc.sha256 not in seen_hashes:
                ...
                logger.info(f"Filtering duplicate document: {doc.title} (hash: {doc.sha256[:8]}...)")
    ...
    async def process_source(self, params: Dict[str, Any]) -> List[DocumentLike]:
        ...
        try:
            ...
            for raw_doc in raw_docs:
                if not self._is_duplicate(raw_doc.get("text", "") or raw_doc.get("content", "")):
                    doc = self._normalize_document(
                        raw_doc, 
                        self.source_type,
                        params.get("transcript_mode")
                    )
            ...
            logger.info(f"Processed {len(raw_docs)} documents, {len(final_docs)} unique after deduplication")
            return final_docs
