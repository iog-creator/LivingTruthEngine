...
try:
    from ..ingestion_general.adapters.youtube_adapter import (
...
class YouTubeAdapter(BaseAdapter):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if LEGACY_YOUTUBE_AVAILABLE:
            ...
            logger.warning(
                "Legacy YouTube adapter not available, using mock implementation"
            )
    ...
    async def _fetch_with_legacy_adapter(
        ...
        try:
            ...
            videos = self.legacy_adapter.discover_videos(
                channel_url=channel_url, selection=sort, max_videos=limit
            )
    ...
    async def _fetch_transcript(self, video_id: str, mode: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error(
                f"Error fetching transcript for {video_id} with mode {mode}: {e}"
            )
            return None
    ...
    async def _fetch_mock_documents(
        ...
        for i in range(min(limit, 3)):  # Max 3 mock documents
            doc = {
                ...
                "text": f"This is mock transcript content for video {i + 1}. It contains sample text that would normally be extracted from a YouTube video transcript using {transcript_mode} mode.",  # noqa: E501
                "metadata": {
                    "video_id": f"mock_{i}",
