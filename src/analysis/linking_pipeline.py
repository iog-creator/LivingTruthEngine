...
class LinkingPipeline:
    ...
    def _init_models(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"✅ Linking pipeline initialized with embedding dim: {self.embedding_dim}"  # noqa: E501
            )
    ...
    def extract_entities(self, doc: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.info(
                f"Extracted {len(entities)} entities from document {doc.get('id', 'unknown')}"  # noqa: E501
            )
            return entities
    ...
    def extract_claims(self, doc: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.info(
                f"Extracted {len(claims)} claims from document {doc.get('id', 'unknown')}"  # noqa: E501
            )
            return claims
    ...
    def embed_entities_claims(
        self, entities: List[Dict[str, Any]], claims: List[Dict[str, Any]]
        ...
        try:
            if not self.embedder:
                ...
                entity_embeddings = [
                    np.random.randn(self.embedding_dim) for _ in entities
                ]
            ...
            entity_embeddings = (
                self.embedder.encode(entity_texts) if entity_texts else []
            )
            claim_embeddings = self.embedder.encode(claim_texts) if claim_texts else []

            self.logger.info(
                f"Generated embeddings: {len(entity_embeddings)} entities, {len(claim_embeddings)} claims"  # noqa: E501
            )
            return entity_embeddings, claim_embeddings
            ...
            for i, entity1 in enumerate(entities):
                for j, entity2 in enumerate(entities[i + 1 :], i + 1):
                    ...
                    if (
                        ...
                        and self._string_similarity(entity1["value"], entity2["value"])
                        > 0.7
                    ):
                        link = {
            ...
            for i, claim1 in enumerate(claims):
                for j, claim2 in enumerate(claims[i + 1 :], i + 1):
                    ...
                    similarity = self._string_similarity(
                        claim1["normalized"], claim2["normalized"]
                    )
                    if similarity > 0.6:
    ...
    def snapshot_graph(self, run_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                # For Phase 9.3, use mock findings since async calls are complex in this context  # noqa: E501
                rulego_findings = {
                    "status": "ok",
                    "findings": [
                        {
            ...
            try:
                ...
                corroboration_results = corroboration_program.batch_verify(
                    run_id, claims_with_evidence
                )
            except Exception as e:
            ...
            graph = {
                ...
                "edges": {"entity_links": entity_links, "claim_links": claim_links},
                "findings": {
            ...
            self.logger.info(
                f"Graph snapshot created for run {run_id}: {graph['metadata']}"
            )
            return graph
    ...
    def process_run(
        self, run_id: str, documents: List[Dict[str, Any]], rebuild: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            self.logger.info(
                f"Starting linking pipeline for run {run_id} with {len(documents)} documents (rebuild={rebuild})"  # noqa: E501
            )
            ...
            try:
                ...
                for doc in documents:
                    ...
                    # Generate embeddings
                    entity_embeddings, claim_embeddings = self.embed_entities_claims(
                        entities, claims
                    )
                ...
                self.logger.info(
                    f"Graph build completed for run {run_id} in {duration_ms}ms: {insert_counts}"  # noqa: E501
                )

                # Record performance event for health monitoring
                self._record_graph_build_event(
                    run_id, insert_counts, upsert_counts, conflict_counts, duration_ms
                )
                ...
                self.logger.info(
                    f"Linking pipeline completed for run {run_id}: {results}"
                )
                return results
                ...
                duration_ms = int((time.time() - start_time) * 1000)
                self.logger.error(
                    f"Linking pipeline failed for run {run_id} after {duration_ms}ms, rolling back: {e}"  # noqa: E501
                )
                raise
            ...
            duration_ms = int((time.time() - start_time) * 1000)
            self.logger.error(
                f"Linking pipeline failed for run {run_id} after {duration_ms}ms: {e}"
            )
            return {
                "run_id": run_id,
                "status": "failed",
