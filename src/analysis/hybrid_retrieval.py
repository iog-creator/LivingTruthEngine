...
class LMStudioEmbeddings(Embeddings):
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        for i, text in enumerate(texts):
            ...
            logger.info(
                f"Successfully embedded document {i + 1} with {len(embedding)} dimensions"  # noqa: E501
            )
...
class BiblicalReranker:
    ...
    def _calculate_biblical_score(self, document: Document, query: str) -> float:
        ...
        final_score = base_score * 0.4 + pattern_score * 0.4 + query_relevance * 0.2
...
class HybridRetriever:
    """Hybrid retriever combining vector search, keyword search, and Biblical reranking."""  # noqa: E501
    ...
    def retrieve_documents(self, query: str, top_k: int = None) -> List[Document]:
        ...
        try:
            ...
            reranked_documents = self.biblical_reranker.rerank_documents(
                documents, query
            )
    ...
    def _load_documents_from_sources(self) -> List[Document]:
        ...
        try:
            for file_path in sources_dir.rglob("*"):
                if (
                    file_path.is_file()
                    and file_path.suffix in self.config.processing.SUPPORTED_EXTENSIONS
                ):
                    try:
    ...
    def _extract_biblical_evidence_from_document(
    ...
    def _extract_survivor_testimony(
        self, document: Document, query: str
    ) -> Optional[Dict[str, Any]]:
...
class AdvancedSearchEngine:
    ...
    def search(
        self, query: str, search_type: str = "hybrid", top_k: int = None
    ) -> Dict[str, Any]:
    ...
    def get_search_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_used_search_type": max(self.search_stats.items(), key=lambda x: x[1])[
                0
            ]
