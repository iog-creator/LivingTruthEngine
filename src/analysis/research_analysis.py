...
class Claim:
    ...
    verification_status: str  # 'unverified', 'verified', 'disputed', 'partially_verified'
...
class ResearchAnalysisSystem:
    ...
    def extract_claims_from_transcript(self, transcript_data: Dict[str, Any]) -> List[Claim]:
        ...
        for i, sentence in enumerate(sentences):
            ...
            if confidence > 0.3:
                claim = Claim(
                    ...
                    source_timestamp=None,  # Could be enhanced with timestamp extraction
    ...
    def calculate_claim_confidence(self, text: str, entities: List[Tuple[str, str]]) -> float:
        ...
        confidence_keywords = [
            'said', 'stated', 'claimed', 'reported', 'witnessed',
            'saw', 'heard', 'experienced', 'knew', 'confirmed'
        ]
        
        text_lower = text.lower()
        keyword_matches = sum(1 for keyword in confidence_keywords if keyword in text_lower)
        confidence += min(keyword_matches * 0.1, 0.3)
        
        # Penalty for uncertainty indicators
        uncertainty_indicators = ['maybe', 'possibly', 'perhaps', 'might', 'could', 'allegedly']
        uncertainty_matches = sum(1 for indicator in uncertainty_indicators if indicator in text_lower)
    ...
    def build_entity_network(self):
        ...
        for rel_id, relationship in self.relationships.items():
            if relationship.entity1_id in self.entities and relationship.entity2_id in self.entities:
                self.graph.add_edge(relationship.entity1_id, 
                                  relationship.entity2_id,
                                  relationship_type=relationship.relationship_type,
                                  confidence_score=relationship.confidence_score,
                                  verification_status=relationship.verification_status)
        
        logger.info(f"✅ Built entity network with {self.graph.number_of_nodes()} nodes and {self.graph.number_of_edges()} edges")
    ...
    def build_relationship_graph(self):
        ...
        for claim in self.claims.values():
            ...
            for i, (entity1_text, entity1_label) in enumerate(entities):
                for j, (entity2_text, entity2_label) in enumerate(entities[i+1:], i+1):
                    entity1_id = f"{entity1_label}_{entity1_text.lower().replace(' ', '_')}"
                    entity2_id = f"{entity2_label}_{entity2_text.lower().replace(' ', '_')}"
        ...
        logger.info(f"✅ Built relationship graph with {len(self.entities)} entities and {len(self.relationships)} relationships")
    ...
    def _generate_network_viz(self, viz_dir: Path):
        ...
        # Save plot
        network_file = viz_dir / f"network_graph_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
    ...
    def _generate_claims_timeline(self, viz_dir: Path):
        ...
        # Save timeline data
        timeline_file = viz_dir / f"claims_timeline_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def _generate_entity_stats(self, viz_dir: Path):
        ...
        # Save statistics
        stats_file = viz_dir / f"entity_stats_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def get_analysis_summary(self) -> Dict[str, Any]:
        ...
        return {
            'claims': {
                ...
                'avg_confidence': sum(c.confidence_score for c in self.claims.values()) / len(self.claims) if self.claims else 0
            },
            'entities': {
                'total': len(self.entities),
                'by_type': defaultdict(int),
                'by_risk_level': defaultdict(int),
                'avg_verification_score': sum(e.verification_score for e in self.entities.values()) / len(self.entities) if self.entities else 0
            },
            'relationships': {
...
class ResearchAnalysisGUI:
    ...
    def setup_claims_tab(self, notebook):
        ...
        self.claims_tree = ttk.Treeview(claims_list_frame, columns=columns, show='headings')
        ...
        # Scrollbar
        claims_scrollbar = ttk.Scrollbar(claims_list_frame, orient='vertical', command=self.claims_tree.yview)
        ...
        ttk.Button(buttons_frame, text="Refresh", command=self.refresh_claims).pack(side='left', padx=5)
        ttk.Button(buttons_frame, text="Export", command=self.export_claims).pack(side='left', padx=5)
        ttk.Button(buttons_frame, text="Filter", command=self.filter_claims).pack(side='left', padx=5)
        ttk.Button(buttons_frame, text="Mark Verified", command=self.mark_verified).pack(side='left', padx=5)
        ttk.Button(buttons_frame, text="Mark Unverified", command=self.mark_unverified).pack(side='left', padx=5)
        ttk.Button(buttons_frame, text="Confidence Stats", command=self.show_confidence_stats).pack(side='left', padx=5)
    ...
    def setup_entities_tab(self, notebook):
        ...
        self.entities_tree = ttk.Treeview(entities_list_frame, columns=columns, show='headings')
        ...
        # Scrollbar
        entities_scrollbar = ttk.Scrollbar(entities_list_frame, orient='vertical', command=self.entities_tree.yview)
        ...
        ttk.Button(buttons_frame, text="Refresh", command=self.refresh_entities).pack(side='left', padx=5)
        ttk.Button(buttons_frame, text="Export", command=self.export_entities).pack(side='left', padx=5)
        ttk.Button(buttons_frame, text="Filter", command=self.filter_entities).pack(side='left', padx=5)
        ttk.Button(buttons_frame, text="Risk Assessment", command=self.analyze_risk).pack(side='left', padx=5)
        ttk.Button(buttons_frame, text="Entity Stats", command=self.show_entity_stats).pack(side='left', padx=5)
    ...
    def setup_relationships_tab(self, notebook):
        ...
        # Relationships list
        relationships_list_frame = ttk.LabelFrame(relationships_frame, text="Relationships")
        ...
        # Scrollbar
        relationships_scrollbar = ttk.Scrollbar(relationships_list_frame, orient='vertical', command=self.relationships_tree.yview)
        ...
        ttk.Button(buttons_frame, text="Refresh", command=self.refresh_relationships).pack(side='left', padx=5)
        ttk.Button(buttons_frame, text="Export", command=self.export_relationships).pack(side='left', padx=5)
        ttk.Button(buttons_frame, text="Filter", command=self.filter_relationships).pack(side='left', padx=5)
        ttk.Button(buttons_frame, text="Relationship Stats", command=self.show_relationship_stats).pack(side='left', padx=5)
    ...
    def setup_network_tab(self, notebook):
        ...
        ttk.Button(controls_frame, text="Generate Network", command=self.generate_network).pack(side='left', padx=5)
        ttk.Button(controls_frame, text="Load Visualization", command=self.load_visualization).pack(side='left', padx=5)
        ttk.Button(controls_frame, text="Save Visualization", command=self.save_visualization).pack(side='left', padx=5)
        ...
        ttk.Button(controls_frame, text="Network Stats", command=self.show_network_stats).pack(side='left', padx=5)
    ...
    def setup_analysis_tab(self, notebook):
        ...
        ttk.Button(controls_frame, text="Run Full Analysis", command=self.run_full_analysis).pack(side='left', padx=5)
        ...
        ttk.Button(controls_frame, text="Search Claims", command=self.search_claims).pack(side='left', padx=5)
        ttk.Button(controls_frame, text="Risk Assessment", command=self.risk_assessment).pack(side='left', padx=5)
        ttk.Button(controls_frame, text="Export All Data", command=self.export_all_data).pack(side='left', padx=5)
