...
class BiblicalForensicConfig:
    ...
    FORENSIC_INFERENCE_CONFIDENCE: float = (
        0.6  # Ezekiel 16:20-21 - expose generational abuse
    )

    # Biblical References for Verification
    BIBLICAL_ABUSE_REFERENCES: List[str] = field(
    ...
    HISTORICAL_ABUSE_REFERENCES: List[str] = field(
        default_factory=lambda: [
            "Moloch worship",
...
class ModelConfig:
    ...
    LMSTUDIO_EMBEDDING_URL: str = os.getenv(
        "LMSTUDIO_EMBEDDING_URL", "http://localhost:1234/v1/embeddings"
    )
    LMSTUDIO_EMBEDDING_MODEL_QWEN3: str = os.getenv(
        "LMSTUDIO_EMBEDDING_MODEL_QWEN3", "text-embedding-qwen3-embedding-0.6b"
    )
    LMSTUDIO_EMBEDDING_MODEL_MINILM: str = os.getenv(
        "LMSTUDIO_EMBEDDING_MODEL_MINILM", "text-embedding-all-minilm-l6-v2-embedding"
    )
    ...
    # Reranker
    LMSTUDIO_RERANKER_MODEL: str = os.getenv(
...
class HotswapConfig:
    ...
    # Component swapping
    SWAPPABLE_COMPONENTS: List[str] = field(
        default_factory=lambda: [
            "retriever",
            "reranker",
            "embedding_model",
...
class VeritasFlags:
    ...
    suspect_min_chars: int = (
        800  # Minimum characters to consider text extraction successful
    )
    ...
    content_type_whitelist: List[str] = field(
        default_factory=lambda: ["text/html", "application/pdf", "text/plain"]
    )
        ...
        try:
            ...
            if config_path.exists():
                ...
                return cls(
                    ...
                    content_type_whitelist=web.get(
                ...
                print(
                    f"Warning: VeritasFlags TOML file not found at {toml_path}, using defaults"  # noqa: E501
                )
                return cls()
...
class LivingTruthConfig:
    ...
    def _load_environment_variables(self):
        ...
        if os.getenv("BIBLICAL_CONFIDENCE_BASELINE"):
            self.biblical_forensic.SURVIVOR_CONFIDENCE_BASELINE = float(
                os.getenv("BIBLICAL_CONFIDENCE_BASELINE")
            )

        if os.getenv("EVIDENCE_VERIFICATION_THRESHOLD"):
            self.biblical_forensic.EVIDENCE_VERIFICATION_THRESHOLD = float(
                os.getenv("EVIDENCE_VERIFICATION_THRESHOLD")
            )

        if os.getenv("FORENSIC_INFERENCE_CONFIDENCE"):
            self.biblical_forensic.FORENSIC_INFERENCE_CONFIDENCE = float(
                os.getenv("FORENSIC_INFERENCE_CONFIDENCE")
            )

        # Feature flags
        self.ENABLE_AGI_INTEGRATION = (
            os.getenv("ENABLE_AGI_INTEGRATION", "true").lower() == "true"
        )
        self.ENABLE_RESEARCH_ANALYSIS = (
            os.getenv("ENABLE_RESEARCH_ANALYSIS", "true").lower() == "true"
        )
        self.ENABLE_CLAIMS_VERIFICATION = (
            os.getenv("ENABLE_CLAIMS_VERIFICATION", "true").lower() == "true"
        )
        self.ENABLE_ADVANCED_VISUALIZATION = (
            os.getenv("ENABLE_ADVANCED_VISUALIZATION", "true").lower() == "true"
        )
        self.GRAPH_DATABASE_ENABLED = (
            os.getenv("GRAPH_DATABASE_ENABLED", "true").lower() == "true"
        )
    ...
    def get_environment_variables(self) -> Dict[str, str]:
        ...
        return {
            # Biblical forensic
            "BIBLICAL_CONFIDENCE_BASELINE": str(
                self.biblical_forensic.SURVIVOR_CONFIDENCE_BASELINE
            ),
            "EVIDENCE_VERIFICATION_THRESHOLD": str(
                self.biblical_forensic.EVIDENCE_VERIFICATION_THRESHOLD
            ),
            "FORENSIC_INFERENCE_CONFIDENCE": str(
                self.biblical_forensic.FORENSIC_INFERENCE_CONFIDENCE
            ),
            # Database
            ...
            "LMSTUDIO_EMBEDDING_MODEL_MINILM": self.model.LMSTUDIO_EMBEDDING_MODEL_MINILM,  # noqa: E501
            ...
            "ENABLE_ADVANCED_VISUALIZATION": str(
                self.ENABLE_ADVANCED_VISUALIZATION
            ).lower(),
    ...
    def validate_configuration(self) -> bool:
        ...
        try:
            ...
            if not (0.0 <= self.biblical_forensic.SURVIVOR_CONFIDENCE_BASELINE <= 1.0):
                raise ValueError(
                    "SURVIVOR_CONFIDENCE_BASELINE must be between 0.0 and 1.0"
                )

            if not (
                0.0 <= self.biblical_forensic.EVIDENCE_VERIFICATION_THRESHOLD <= 1.0
            ):
                raise ValueError(
                    "EVIDENCE_VERIFICATION_THRESHOLD must be between 0.0 and 1.0"
                )

            if not (0.0 <= self.biblical_forensic.FORENSIC_INFERENCE_CONFIDENCE <= 1.0):
                raise ValueError(
                    "FORENSIC_INFERENCE_CONFIDENCE must be between 0.0 and 1.0"
                )
    ...
    def get_config_summary(self) -> Dict[str, Any]:
        ...
        return {
            "biblical_forensic": {
                "survivor_confidence_baseline": self.biblical_forensic.SURVIVOR_CONFIDENCE_BASELINE,  # noqa: E501
                "evidence_verification_threshold": self.biblical_forensic.EVIDENCE_VERIFICATION_THRESHOLD,  # noqa: E501
                "forensic_inference_confidence": self.biblical_forensic.FORENSIC_INFERENCE_CONFIDENCE,  # noqa: E501
                "biblical_references_count": len(
                    self.biblical_forensic.BIBLICAL_ABUSE_REFERENCES
                ),
                "historical_references_count": len(
                    self.biblical_forensic.HISTORICAL_ABUSE_REFERENCES
                ),
            },
            "database": {
                "postgres_host": self.database.POSTGRES_HOST,
