...
class CanonicalizePipeline:
    ...
    def _create_provenance(self, doc: Dict[str, Any]) -> Dict[str, Any]:
        ...
        provenance = {
            "extraction_method": doc.get("meta", {}).get(
    ...
    def _create_error_document(
        self, original_doc: Dict[str, Any], index: int, error_message: str
    ) -> Dict[str, Any]:
        """Create error document when canonicalization fails"""
        return {
            "id": f"error_{index}_{hashlib.md5(error_message.encode()).hexdigest()[:8]}",  # noqa: E501
            "index": index,
            ...
            "metadata": {"char_count": 0, "word_count": 0, "source_type": "error"},
        }
