...
class ProvenancePipeline:
    ...
    def create_provenance_data(
        self, 
        canonicalized_docs: List[Dict[str, Any]],
        ...
        logger.info(f"Provenance data created with {len(proofs)} proofs and merkle root: {merkle_data.get('root', 'N/A')}")
    ...
    def _generate_metrics(
        ...
        for doc in canonicalized_docs:
            ...
            # Extraction method
            extraction_method = doc.get("provenance", {}).get("extraction_method", "unknown")
            ...
            if source_type == "youtube":
                ...
                if doc.get("text") and not doc.get("text", "").startswith("[No transcript"):
                ...
                web_metrics["max_crawl_depth"] = max(web_metrics["max_crawl_depth"], crawl_depth)
                ...
                pdf_metrics["total_pages"] += doc.get("pdf", {}).get("page_count", 0) or 0
                pdf_metrics["ocr_attempts"] += doc.get("pdf", {}).get("ocr_attempts", 0) or 0
                ...
                for line_num, line in enumerate(f, 1):
                    try:
                        ...
                        if not proof_file.exists():
                            verification_results["verified"] = False
                            verification_results["errors"].append(f"Proof missing for document {doc_id}")
                            continue
                        ...
                        if proof.get("text_hash") != expected_hash:
                            verification_results["verified"] = False
                            verification_results["errors"].append(f"Text hash mismatch for document {doc_id}")
                        
                    except json.JSONDecodeError as e:
                        verification_results["verified"] = False
                        verification_results["errors"].append(f"Invalid JSON in corpus.jsonl line {line_num}: {e}")
                    except Exception as e:
                        verification_results["verified"] = False
                        verification_results["errors"].append(f"Error verifying document at line {line_num}: {e}")
