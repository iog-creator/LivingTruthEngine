...
def fetch_pdf(paths_or_urls: List[str]) -> List[Dict[str, Any]]:
    ...
    for path_or_url in paths_or_urls:
        try:
            ...
            if path_or_url.startswith(('http://', 'https://')) and os.path.exists(tmp_path):
                os.unlink(tmp_path)
            ...
            documents.append({
                "source_type": "pdf",
                "uri": path_or_url,
                "title": Path(path_or_url).stem if not path_or_url.startswith(('http://', 'https://')) else urlparse(path_or_url).path.split('/')[-1],
                "text": text_content,
                "meta": {
                    "pages": len(doc),
...
def fetch_youtube(channel_or_urls: List[str], max_videos: int = 10) -> List[Dict[str, Any]]:
    """
            ...
            for line in lines:
                ...
                if len(parts) >= 3:
                    ...
                    if os.path.exists(subtitle_file):
                        ...
                        import re
                        text_content = re.sub(r'\d+:\d+:\d+,\d+ --> \d+:\d+:\d+,\d+', '', subtitle_content)
