...
class YouTubeAdapter:
    ...
    def discover_videos(
        ...
        """
        logger.info(f"Discovering videos from {channel_url} with selection={selection}, max_videos={max_videos}")
        ...
        cmd = [
            ...
            "--flat-playlist",
            "--no-playlist",
            "--print", "id,title,upload_date,view_count,duration,webpage_url",
        ...
        try:
            ...
            result = subprocess.run(
                ...
                capture_output=True, 
                text=True, 
                timeout=self.extract_flat_timeout
            )
    ...
    def _fetch_specific_videos(self, ids: List[str], max_videos: int) -> List[Dict[str, Any]]:
        """Fetch specific video IDs"""
        videos = []
    ...
    def _parse_yt_dlp_output(self, output: str) -> List[Dict[str, Any]]:
        ...
        for i in range(0, len(lines), 6):
            if i + 5 < len(lines):
                try:
                    video_data = {
                        "id": lines[i].split('v=')[-1] if 'v=' in lines[i] else lines[i],
                        "title": lines[i + 1],
                        "upload_date": lines[i + 2],
                        "view_count": int(lines[i + 3]) if lines[i + 3].isdigit() else 0,
                        "duration": float(lines[i + 4]) if lines[i + 4].replace('.', '').isdigit() else 0,
                        "webpage_url": lines[i + 5],
                        "source_type": "youtube"
                    }
                    videos.append(video_data)
                except Exception as e:
                    logger.warning(f"Failed to parse video group starting at line {i}: {e}")
                    continue
    ...
    def _parse_single_video_output(self, output: str) -> Optional[Dict[str, Any]]:
        ...
        lines = output.strip().split('\n')
        if lines:
            return self._parse_yt_dlp_output(lines[0])[0] if self._parse_yt_dlp_output(lines[0]) else None
        return None
    
    def _apply_selection(self, videos: List[Dict[str, Any]], selection: str, max_videos: int) -> List[Dict[str, Any]]:
    ...
    def get_video_metadata(self, video_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            cmd = [
                "yt-dlp",
                "--extract-flat",
                "--print", "id,title,upload_date,view_count,duration,webpage_url,description,channel,channel_id",
                f"https://www.youtube.com/watch?v={video_id}"
            ]
