...
class VeritasRunner:
    ...
    def start(
        ...
        if not channel_url:
            channel_url = self.flags.get(
                "default_channel", "https://www.youtube.com/@imaginationpodcastofficial"
            )
        ...
        if "youtube" in sources:
            try:
                ...
                videos = self.youtube_adapter.discover_videos(
                    channel_url=channel_url, selection=selection, max_videos=max_videos
                )
        ...
        if "web" in sources and crawl_depth > 0:
            try:
                ...
                web_docs = self.web_fetcher.fetch_url(
                    url=channel_url, crawl_depth=0, max_depth=crawl_depth
                )
        ...
        # Write corpus.jsonl
        corpus_content = self.canonicalize_pipeline.create_corpus_jsonl(
            canonicalized_docs
        )
        ...
        manifest = {
            ...
            or f"{topic} — {max_videos} videos — depth {crawl_depth}",
            "flags": flags,
        ...
        try:
            ...
            for doc in canonicalized_docs:
                db_doc = {
                    "id": doc.get(
                        "id", ""
                    ),  # Use the canonicalized document ID as sha256
            ...
            if db_docs:
                ...
                logger.info(
                    f"Stored {len(db_docs)} documents in database for run {run_id}"
                )
            else:
    ...
    def get_status(self, run_id: str) -> VeritasRunStatus:
        ...
        bundle = next(
            (p for p in self.runs_dir.glob(f"{run_id}.veritasrun") if p.is_dir()), None
        )
        if not bundle:
    ...
    def list_runs(self, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        for bundle_path in sorted(
            self.runs_dir.glob("*.veritasrun"), key=lambda p: p.name, reverse=True
        ):
            ...
            try:
                ...
                if manifest_path.exists():
                    ...
                    if metrics_path.exists():
                        ...
                        doc_count = metrics.get("run_summary", {}).get(
                            "total_documents", 0
                        )
