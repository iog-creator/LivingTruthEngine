...
class PDFExtractor:
    ...
    def extract_text(
        ...
        try:
            ...
            logger.warning(f"Text extraction yielded only {len(text_content)} characters from {filename}")
            ...
            if ocr_mode == "off":
                ...
                ocr_result = self._perform_ocr(pdf_content, max_attempts=self.auto_retry_attempts)
                return {
    ...
    def _perform_ocr(self, pdf_content: bytes, max_attempts: int = 2) -> Dict[str, Any]:
        ...
        try:
            ...
            final_text = "\n\n".join(all_text)
            logger.info(f"OCR completed with {total_attempts} attempts, extracted {len(final_text)} characters")
    ...
    def extract_text_from_file(self, file_path: str, ocr_mode: str = "off") -> Dict[str, Any]:
