...
class WebFetcher:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.content_type_whitelist = config.get(
            "content_type_whitelist", ["text/html", "application/pdf", "text/plain"]
        )
    ...
    def fetch_url(
        ...
        if domain_counts[domain] >= self.max_pages_per_domain:
            logger.info(
                f"Reached max pages per domain limit for {domain} ({self.max_pages_per_domain})"  # noqa: E501
            )
            return documents
    ...
    def _fetch_single_url(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not any(ct in content_type for ct in self.content_type_whitelist):
                logger.warning(
                    f"Content type {content_type} not in whitelist for {url}"
                )
                return None
    ...
    def _extract_html_content(self, url: str, html_content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "text": extracted_text
                or html_content[:1000],  # Fallback to first 1000 chars
                "meta": {
